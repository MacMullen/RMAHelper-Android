...
val COL_IMAGEVIEW = "image"

class DatabaseHandler(context: Context) : SQLiteOpenHelper(context, DATABASE_NAME, null, 1) {
    override fun onCreate(db: SQLiteDatabase?) {

        val createTable =
            "CREATE TABLE " + PRODUCTS_TABLE_NAME + " (" + COL_ID + " INTEGER PRIMARY KEY AUTOINCREMENT," + COL_BRAND + " VARCHAR(256)," + COL_MODEL + " VARCHAR(256)," + COL_DIST_COMPANY + " VARCHAR(256)," + COL_EAN + " VARCHAR(256)," + COL_ACCESSORIES + " VARCHAR(256)," + COL_IMAGEVIEW + " VARCHAR(256))"
    ...
    fun insertProduct(product: Product): Long {
        ...
        cv.put(COL_IMAGEVIEW, product.image.toString())
